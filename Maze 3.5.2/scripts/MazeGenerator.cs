...
public class MazeGenerator : TileMap
{
    ...
    List<RouteNode> nodeList = new List<RouteNode>(); //for nodes,maybe get rid of this to be honest
    IDictionary<Vector2, RouteNode> nodeDict = new Dictionary<Vector2, RouteNode>(); //for nodes, most likely get rid of this tbh and replace with node class
    ...
    private void AddNode(Vector2 nodeLocation)
    {
        ...
        {
            RouteNode newNode = new RouteNode(nodeId++, nodeLocation);
            ...
            nodeDict.Add(nodeLocation, newNode); //TBH theres probably a better way to do this...
        }
        else
        {
    ...
    private Vector2[,] GenerateAdjMatrix()
    {
        ...
        for (int i = 0; i < nodeList.Count; i++) //arrays go y,x, meaning i = y downwards, j = x rightwards
        {
            ...
            {
                if ((nodeList[i].pos.x == nodeList[j].pos.x) || (nodeList[i].pos.y == nodeList[j].pos.y))
                {
                    adjMatrix[i, j] = nodeList[i].pos - nodeList[j].pos; //what we get is node1.pos - adjMatrix[i,j] == node2.pos
                }
                else
                {
                    adjMatrix[i, j] = Vector2.Zero; //0,0 means no relationship
                }
            }
        }
